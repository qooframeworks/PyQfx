...
class QfxPriceSeasonal:
	...
	def get_time_first(self) -> tuple:
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_shift_time_first.sql")		# SQL 文をロード
	...
	def get_time_last(self) -> tuple:
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_shift_time_last.sql")		# SQL 文をロード
		str_sql: str = ary_sql[0]																		# SQL 文を取得
		str_sql = str_sql.format(																		# SQL 文を構成
			strSymbol=self.m_qfxPriceConfig.strSymbol,													# 銘柄名
			strTimeFrames=self.m_qfxPriceConfig.strTimeFrames,											# 時間足
	...
	def load_source(
		...
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_value.sql")		# SQL 文をロード
		str_sql: str = ary_sql[0]															# SQL 文を取得
		str_sql = str_sql.format(															# SQL 文を構成
			strSymbol=self.m_qfxPriceConfig.strSymbol,										# 銘柄名
			strTimeFrames=self.m_qfxPriceConfig.strTimeFrames,								# 時間足
	...
	def delete(
		...
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_delete.sql")		# SQL 文をロード
		str_sql: str = ary_sql[0]															# SQL 文を取得
		str_sql = str_sql.format(															# SQL 文を構成
			strSymbol=self.m_qfxPriceConfig.strSymbol,										# 銘柄名
			strTimeFrames=self.m_qfxPriceConfig.strTimeFrames,								# 時間足
	...
	def delete_all(self) -> bool:
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_delete_all.sql")		# SQL 文をロード
		str_sql: str = ary_sql[0]																# SQL 文を取得
		str_sql = str_sql.format(																# SQL 文を構成
			strSymbol=self.m_qfxPriceConfig.strSymbol,											# 銘柄名
			strTimeFrames=self.m_qfxPriceConfig.strTimeFrames,									# 時間足
	...
	def insert(
		...
		str_shift_time_right: str = dt_shift_right.strftime('%Y-%m-%d %H:%M:%S')			# 右端シフト日時を文字列に変換
		str_shift_time: str = dt_shift.strftime('%Y-%m-%d %H:%M:%S')						# 算定シフト日時を文字列に変換
		ary_sql: list[str] = QfxMySql.QfxMySql.get_sql("Sql/price_seasonal_insert.sql")		# SQL 文をロード
		str_sql: str = ary_sql[0]															# SQL 文を取得
		str_sql = str_sql.format(															# SQL 文を構成
			strSymbol=self.m_qfxPriceConfig.strSymbol,										# 銘柄名
			strTimeFrames=self.m_qfxPriceConfig.strTimeFrames,								# 時間足
